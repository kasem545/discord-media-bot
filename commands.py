...
async def process_send_media(ctx, media_type, comment, channel_name, attachment):
    if attachment is None:
        await ctx.send("Please attach a media file.", ephemeral=True)
        return

    if media_type.lower() == "image":
        if not attachment.content_type.startswith('image'):
            await ctx.send("Please attach a valid image file.", ephemeral=True)
            return
    elif media_type.lower() == "video":
        if not attachment.content_type.startswith('video'):
            await ctx.send("Please attach a valid video file.", ephemeral=True)
            return
    else:
        await ctx.send("Invalid media type. Use 'image' or 'video'.", ephemeral=True)
        return
    ...
    if channel is None:
        await ctx.send(f"Could not find the specified channel '{channel_name}'. Please ensure the channel exists.", ephemeral=True)
        return
        
    await channel.send(f"{ctx.user.mention} {media_url}\n comment: {comment}")
    await asyncio.sleep(5)
    await ctx.send(f"{ctx.user.mention} Media sent successfully :)", ephemeral=True)
...
async def options(ctx):
    ...
    await ctx.send(options_text, ephemeral=True)
